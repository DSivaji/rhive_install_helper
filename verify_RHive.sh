...
Rscript -e 'tmp_table_name<-sample(1:1000000, 1);tmp_table_name<-paste("tmp_",tmp_table_name,sep="");library(RHive);rhive.connect('$THRIFT_ADDRESS');rhive.query("set hive.fetch.task.conversion=minimal");rhive.write.table(tablename=tmp_table_name,USArrests);sumCrimes <- function(column1, column2, column3) {return(1)};rhive.assign("sumCrimes", sumCrimes);rhive.export("sumCrimes");rhive.query(paste("SELECT sum(R(\"sumCrimes\", murder, assault, rape, 0.0)) FROM ",tmp_table_name, " limit 1",sep=""));rhive.drop.table(tmp_table_name)'  > $RN_ck_FILE 2> /dev/null
	...
	#echo "return for UDF "$RES2
        ...
        if [ "$RES2" == "50" ]; then
              echo -e "RHive UDF function is working -  ${txtgrn}OK${txtrst}"
        else
              echo -e "RHive UDF is not working -  ${txtred}FAILED${txtrst}"
        fi
